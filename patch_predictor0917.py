...
class PatchCorrectnessPredictor:
    ...
    def __init__(self, model='gpt-3.5-turbo-instruct', max_context_tokens=None, temperature=0, api_key="", top_k=10, models_cache_dir="./models_cache"):
        ...
        if model.startswith('gpt-'):
            ...
            try:
                ...
                #self.max_context_tokens = 2048 - 16
                self.model_type = "huggingface"
                self.generation_config = GenerationConfig(
                    max_new_tokens= 16,
                    do_sample=False,
    ...
    def _truncate_by_tokens(self, text: str, limit: int) -> str:
        ids = self.tokenizer.encode(text)
        if len(ids) <= limit:
            return text
        # 对于大模型 tokenizer，直接 decode 子序列能保持基本一致性
        return self.tokenizer.decode(ids[:limit], skip_special_tokens=True)
    ...
    def _build_patch_prompt(self, patch):
        ...
        if functions:
            for i, func in enumerate(functions):
                ...
                prompt += "```diff\n"
                 # 使用 extract_diff_lines 来精简补丁内容
                full_patch_function = func.get('patch_function', '')                
                concise_diff = SimilarityScore.extract_diff_lines(full_patch_function, max_lines=50)  # 限制最大行数
                prompt += concise_diff
                prompt += "```\n"
        else:
    ...
    def _build_evaluation_prompt(self, query_patch, similar_patches):
        ...
        base_prompt = "\n# Let's check patch correctness. 'WRONG' means although it can pass test cases but cannot fix the bug. 'CORRECT' means it really addresses the bug.\n"
       ...
       # --------------------------------------------------------------------
        ...
        if bug_description:
            base_prompt += "\n## The bug refers to:\n"
            # 如果 bug_description 是列表，将其合并为字符串
        ...
        if execution_traces:
            ...
            trace_lines = []
            for trace in execution_traces:  # 遍历 list 中的每个字符串
        ...
        if similar_patches:
            ...
            for i, patch in enumerate(similar_patches):
                ...
                # 使用分词器检查 token 数量
                if len(self.raw_tokenizer.encode(current_prompt_str)) > self.max_context_tokens - 1000:
                    # 如果超限，就不再添加更多的相似补丁，并跳出循环
        ...
        # 添加查询补丁
        final_prompt = '\n This is query patch. Q: It was wrong or correct? A: It was %s'
    ...
    def predict_single_patch(self, query_patch):
        ...
        Args:
            query_patch (dict): 带有similar_patches的查询补丁
            
        Returns:
            tuple: (补丁ID, 预测标签, 原始输出, 原始标签)
        """
    ...
    def predict_patches(self, query_patches_with_similar):
        ...
        Args:
            query_patches_with_similar (list): 带有similar_patches的查询补丁列表
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='codellama-7b-hf', 
                        choices=['gpt-4-turbo', 'codegen2-1b', 'codellama-7b-hf', 'codellama-13b',
